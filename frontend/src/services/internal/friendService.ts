...
export const friendService = {
	async sendRequest(receiverId: string, senderId: string): Promise<void> {
		return callApi('friends', { method: 'POST', body: { receiverId } }, senderId);
	},
	...
	async getStatus(userAId: string, userBId: string): Promise<'friends' | 'pending' | 'none'> {
		const userAFriends = await this.getAllFriends(userAId);
		if (userAFriends.some((friend) => friend.firstUser === userBId || friend.secondUser === userBId)) {
			return 'friends';
		}
		const userAFriendRequests = await this.getAllFriendRequests(userAId);
		if (userAFriendRequests.some((request) => request.userRequested === userBId || request.userPending === userBId)) {
			return 'pending';
		}
		return 'none';
	}
